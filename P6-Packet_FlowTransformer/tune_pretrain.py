...
from train.train import _balanced_loss # Import from train.py

# --- Configuration & Basic Setup ---
CONFIG_FILE_PATH = Path("config.json")
LOG_FILE_OPTUNA_PRETRAIN = "optuna_pretrain_log.txt"

# This is the main logger for the tune_pretrain.py script itself
script_logger = logging.getLogger(__name__) # Renamed from 'logger' to avoid conflict
logging.basicConfig(
    level=logging.INFO,
...
def objective_pretrain(trial: optuna.trial.Trial):
    ...
    if d_model % num_heads != 0:
        script_logger.warning(f"d_model ({d_model}) not divisible by num_heads ({num_heads}). Pruning trial.")
    ...
    script_logger.info(f"Trial {trial.number} Hyperparameters: LR={lr:.2e}, BatchSize={batch_size}, "
    ...
    if train_size + val_size > total_size or train_size <= 0 or val_size <= 0:
        script_logger.warning("Problem with dataset split sizes for pretrain, adjusting.")
    ...
    if train_size <= 0 or val_size <= 0 or dummy_test_size < 0:
        script_logger.error("Cannot create valid train/val splits for pretrain. Aborting trial.")
        return -float('inf')
    ...
    # Call the imported _balanced_loss and pass the silent_logger
    ...
    for epoch in range(1, epochs_pretrain + 1):
        ...
        if trial.should_prune():
            script_logger.info(f"Trial {trial.number} (Pretrain) pruned at epoch {epoch}.")
...
if __name__ == "__main__":
    ...
    optuna_own_logger = optuna.logging.get_logger("optuna") # Renamed to avoid conflict
    optuna_own_logger.addHandler(optuna_stream_handler)
    optuna_own_logger.addHandler(optuna_file_handler)
    optuna_own_logger.setLevel(logging.INFO)

    script_logger.info(f"Starting Optuna study for Pre-training: {study_name_pretrain}. Results: {storage_name_pretrain}")
