...
from pipeline.config import categorical_columns_flows, categorical_columns
...
def generate_and_save_category_mappings(
    ...
    # Choose the appropriate categorical columns and output filename
    ...
    for col in categorical_columns:
        ...
        raw_vals = sorted(raw_vals, key=lambda x: str(x))  # Sort for consistency
        raw_vals.append("unknown")

        mapping = {repr(val): idx for idx, val in enumerate(raw_vals)}
        mappings[col] = mapping

        # Sanity check: ensure 'unknown' is last
