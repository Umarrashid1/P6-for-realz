...
try:
    ...
    if not study.trials:
        ...
        # Define the parameters that were tuned for THIS study (baseline)
        tuned_params = [ # CHANGED
            "lr_baseline_flow",
            "batch_size_baseline_flow",
            "d_model",
            "num_heads",
            "num_layers",
        ...
        # 3. Slice Plot
        try:
            # Check if tuned_params exist in the study to avoid errors if some trials failed before setting all params
        ...
        try:
            valid_parallel_params = [p for p in tuned_params if any(p in t.params for t in completed_trials if t.params)]
        ...
        if len(tuned_params) >= 2:
            contour_pairs = [
                ("lr_baseline_flow", "dropout_transformer_body"),
                ("d_model", "num_layers"),
                ("batch_size_baseline_flow", "lr_baseline_flow"),
                ("d_model", "classifier_dropout_flow")
            ]
            for i, pair in enumerate(contour_pairs):
                param1_in_study = any(trial.state == optuna.trial.TrialState.COMPLETE and pair[0] in trial.params for trial in study.trials if trial.params)
                param2_in_study = any(trial.state == optuna.trial.TrialState.COMPLETE and pair[1] in trial.params for trial in study.trials if trial.params)
                ...
                if param1_in_study and param2_in_study and len(completed_trials) > 1:
                    ...
                    print(f"Skipping contour plot for {pair[0]} vs {pair[1]} due to missing params or insufficient completed trials.")
        else:
