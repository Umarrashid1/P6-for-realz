...
INPUT_DIM_NUMERICAL_PACKET = len(
    numerical_columns_packets)  # Or from config: model_arch_cfg.get('input_dim_packet_numerical', len(numerical_columns_packets))
NUM_CLASSES_PACKET = model_arch_cfg['num_classes_packet']

logger.info(
    f"Key Model Arch Params: embed_dim/d_model={transformer_body_cfg['d_model']}, heads={transformer_body_cfg['num_heads']}, layers={transformer_body_cfg['num_layers']}, dropout={transformer_body_cfg['dropout']}")
logger.info(
    f"Packet Specific Arch Params: max_seq_len={MAX_SEQ_LEN_PACKET}, num_numerical_features={INPUT_DIM_NUMERICAL_PACKET}, num_classes={NUM_CLASSES_PACKET}")
...
logger.info(
    f"1. Loading packet sequence dataset from: {PROCESSED_PACKET_DATA_PATH} with max_seq_len={MAX_SEQ_LEN_PACKET}")
try:
...
try:
    ...
    cat_map_path = config.get('paths', {}).get('category_mappings_packet', 'category_mappings_packets.json')  # Example
    logger.info(f"   Loading packet category mappings from: {cat_map_path}")
    cat_map = load_mappings(path=cat_map_path, is_flow=False)
    cat_sizes = {col: len(cat_map[col]) for col in categorical_columns_packets if col in cat_map}
    cat_pad = {c: cat_map[c]["unknown"] for c in categorical_columns_packets if
               c in cat_map and "unknown" in cat_map[c]}
    ...
    for col in categorical_columns_packets:
        if col not in cat_sizes:
            logger.warning(
                f"Column {col} from categorical_columns_packets not found in loaded category mappings. Check mapping file or column list.")
            ...
            logger.warning(
                f"'unknown' category not found for column {col} in mappings. Using padding_idx 0 as default.")
...
try:
    ...
    if not best_model_path_pretrain.exists():
        logger.warning(
            f"Best model checkpoint not found at {best_model_path_pretrain}. Testing with current model state.")
...
logger.info("🏁 Packet pre-training script (with central config & logging) finished. 🏁")# pretrain_packet_model.py
import os
import torch
from torch.utils.data import random_split
import json
